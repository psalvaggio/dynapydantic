...
def direct_children_of_base_in_mro(derived: type, base: type) -> list[type]:
    ...
    """
    return [cls for cls in derived.__mro__
            if cls is not base and base in cls.__bases__]
...
class DynamicBaseModel(pydantic.BaseModel):
    ...
    def __init_subclass__(cls, *args, discriminator_field: str | None = None, **kwargs):
        super().__init_subclass__(*args, **kwargs)

    @classmethod
    def __pydantic_init_subclass__(cls, *args, discriminator_field: str | None = None, **kwargs):
        if DynamicBaseModel in cls.__bases__:
            if discriminator_field is None:
                msg = (
                    "Direct children of DynamicBaseModel must pass "
                    "discriminator_field"
                )
    ...
    def union(cls):
        if DynamicBaseModel not in cls.__bases__:
            msg = (
                "union() can only be called on direct children of "
                "DynamicBaseModel" 
            )
            raise TypeError(msg)
        ...
        return ty.Annotated[
            ty.Union[
                tuple(
                    (ty.Annotated[x, pydantic.Tag(v)] for v, x in cls.__SUBCLASSES__.items()))
            ],
